...
def test_request_form_form():
    from src.util.request import get_post_body

    request = MockRequest(form={"foo": "bar"}, json=None)
...
def test_request_form_json():
    ...
    request = MockRequest(form={}, json={"foo": "bar"})
    assert get_post_body(request) == {"foo": "bar"}


def test_verify_body_keys_mismatch_error():
    from src.util.request import verify_body_keys
