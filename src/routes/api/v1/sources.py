...
def get_source(source_key):
    ...
    if source_key[:14] == "source-custom-":
        api_key = request.headers.get("X-Api-Key")
        if api_key is None or not verify_access(
            api_key, f"source-custom-{source_key[14:]}"
        ):
...
def engine_users(source_key):
    ...
    users = [
        User.find_user(user_email).api_values for user_email in users_for_engine(engine)
    ]
    return users
