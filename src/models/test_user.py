...
def test_clean_dict():
    ...
    assert set(user.clean_dict.keys()) == set(
        expected_keys
    ), "Clean dict should only contain these keys"
...
def test_search_api_key():
    ...
    assert (
        user.search_api_key == "private-123"
    ), "SearchApiKey should be formatted correctly"
...
def test_document_access_token():
    ...
    with time_machine.travel("2021-01-01 23:59:59"):
        assert (
            user.decode_document_access_token(token) == "private-123"
        ), "Document access token should be decoded correctly"
